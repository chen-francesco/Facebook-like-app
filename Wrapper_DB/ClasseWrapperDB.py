...
class WrapperDB:
    ...
    def elencoPost(self, as_dict = False):
        ...
        try:
            ...
            sql = "SELECT PostId, RootId, Autore, Testo, [Like], Commenti FROM CHEN_FB_Post ORDER BY [Like] DESC"
            cur.execute(sql)
            lista = cur.fetchall()
        except:
    ...
    def singoloPost(self, id):
        ...
        try:
            ...
            sql = f"""
                SELECT PostId, RootId, Autore, Testo, [Like], Commenti
                FROM CHEN_FB_Post 
                WHERE PostId = {id}   
                """
            cursore.execute(sql)
    ...
    def inserisciPost(self, parametri):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "INSERT INTO CHEN_FB_Post (RootId, Autore, Testo) VALUES (%d, %s, %s)"
    ...
    def daiLikeAPost(self, id, is_like = True):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "UPDATE CHEN_FB_Post SET [Like] = "
            if is_like == True: 
                sql += "[Like] + 1 "
            else:
                sql += "[Like] - 1 "
    ...
    def eliminaPost(self, id):
        ...
        try:
            c = self.connetti() 
            cursore = c.cursor()
            sql = "DELETE CHEN_FB_Post WHERE PostId = %d"
            cursore.execute(sql, id)
            c.commit()
